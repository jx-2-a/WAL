...
- 如有需要，主动用 add_chapter、add_character、add_plot_line 扩充故事
...
class AgentLoop:
    ...
    def _update_tools_for_mode(self) -> None:
        ...
        if self.mode == AgentMode.PLANNING:
            # 规划模式：规划工具（分析/创意）
            self.tools = PLAN_TOOL_DEFINITIONS
        elif self.mode == AgentMode.AUTONOMOUS:
            # 自主模式：自主工具 + 写作工具（去重，避免 switch_mode 等跨模式工具重复）
    ...
    def run_turn(self, user_input: str, stream_callback: Callable[[str], None] | None = None) -> str:
        ...
        for _ in range(max_tool_rounds):
            ...
            response = self.llm.chat_with_tools(
                self.messages, self.tools, temperature=0.7, max_tokens=8192
            )
            ...
            # 有 tool_calls 但没有 content：继续循环
            if tool_calls and not content:
                continue

        # 超时：强制 LLM 总结
        self.messages.append({"role": "user", "content": "请根据以上工具调用结果，给出最终回复。"})
        final = self.llm.chat_with_tools(self.messages, self.tools, temperature=0.7, max_tokens=8192)
    ...
    def run_turn_stream(self, user_input: str) -> str:
        ...
        for _ in range(max_tool_rounds):
            ...
            for event in self.llm.chat_stream(self.messages, self.tools, temperature=0.7, max_tokens=8192):
                if event["type"] == "text":
            ...
            if tool_calls_result:
                self._handle_tool_calls(tool_calls_result)
                continue
    ...
    def _handle_tool_calls(self, tool_calls: list[dict]) -> None:
        ...
        for tc in tool_calls:
            ...
            # 根据模式选择工具执行器
                ...
                result = execute_plan_tool(tool_name, args, self.project_name)
            elif self.mode == AgentMode.AUTONOMOUS:
                # 自主模式：先查自主工具，再查写作工具
