...
- ⚠️ **字数管理**：每章有目标字数（word_count_target，默认 3000）。默认值只是一个起点——**你应根据章节在故事中的位置、情节密度和节奏需求，主动设定合适的目标**。高潮章节可以设 4000-6000，过渡章节 1500-2500 即可。用 `add_chapter` 创建时指定 word_count_target，写完后发现目标不合理就用 `update_chapter_info` 调整。**叙事完整性优先于字数指标**：内容写到位了就停，宁可 1500 字的饱满章节，不要 3000 字的水文。灌水凑数是唯一禁止的行为
